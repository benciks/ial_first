...
void List_Dispose( List *list ) {
	...
	// Iterate over the rest of elements and free them
	while (element != NULL) {
		ListElementPtr nextElement = element->nextElement;
		free(element);
		element = nextElement;
	}
...
void List_GetValue( List *list, int *dataPtr ) {
	...
	ListElementPtr element = list->activeElement;
	*dataPtr = element->data;
}
